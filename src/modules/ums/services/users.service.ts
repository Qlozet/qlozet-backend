...
export class UserService {
  ...
  async addToWishlist(userId: string, productId: string): Promise<void> {
    ...
    this.eventsService.logEvent({
      userId,
      eventType: EventType.WISHLIST_ADD, // Ensure this enum exists or mapped
      timestamp: new Date(),
      properties: { itemId: productId },
      context: { surface: 'server_hook' }
    } as any).catch(e => this.logger.warn('Wishlist hook failed', e));
  }
  ...
  async removeFromWishlist(userId: string, productId: string): Promise<void> {
    ...
    // Hook: Wishlist Remove
    this.eventsService.logEvent({
      userId,
      eventType: EventType.WISHLIST_REMOVE,
      timestamp: new Date(),
      properties: { itemId: productId },
      context: { surface: 'server_hook' }
    } as any).catch(e => this.logger.warn('Wishlist remove hook failed', e));
  }
